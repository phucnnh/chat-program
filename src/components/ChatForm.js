...
class ChatForm extends React.Component {
    ...
    while (arr[x] != null) {
      ...
      if (arr[x].email === email) {
        ...
        if (arr[x].email === arr[x].isChattingWith) {
          ...
          if (input != null) {
            {
              imageExists(input, function (exists) {
                if (exists) {
                  ...
                  isImage = true;
                  fiba.push(`message/${emailName[0]}_`, { [name]: `${input}`, isImage: isImage });
                }
                else {
                  console.log("oh well");
                  isImage = false;
          ...
          if (img != null) {
            fiba.push(`message/${emailName[0]}_`, { [name]: `${img}`, isImage: true });
          }
        }
        else {
          var isImage = false;
          ...
          var fiba = this.props.firebase;
          var img = this.props.image;
          var isPathExist = this.isExistPath(
            `${emailName[0]}_${arr[x].isChattingWith.split('@gmail.com')[0]}`, messageStore);
          ...
          if (input != null) {
            {
              imageExists(input, function (exists) {
                if (exists) {
                  ...
                  isImage = true;
                  if (isPathExist) {
                    fiba.push(path1, { [name]: `${input}`, isImage: isImage });
                  }
                  else {
                    fiba.push(path2, { [name]: `${input}`, isImage: isImage });
                  }
                }
                else {
                  console.log("oh well");
          ...
          if (isPathExist) {
            if (img != null) {
              fiba.push(path1, { [name]: `${img}`, isImage: true });
            }
          }
          else {
            if (img != null) {
              fiba.push(path2, { [name]: `${img}`, isImage: true });
            }
          }
  ...
  render() {
    ...
    const userList = Object.keys(swapList).map(
      (key, id) => (
        String(swapList[key].name).toLowerCase().includes(String(this.props.searchlist).toLowerCase()) ?
          (<li key={key} id={id} class="clearfix li-cursor no-bullets" 
          onClick={() => this.clickUser(swapList[key].email)}>
          ...
          if (arr[x].isChattingWith != null) {
            if (arr[x].isChattingWith === arr[y].email) {
              head = (<div class="chat-header clearfix">
                ...
                {arr[x].isStar === arr[y].email ? <i style={starStyle} class="fa fa-star li-cursor" 
      ...
      if (arr[i].email === email) {
        if (arr[i].isChattingWith != null) {
                  ...
                  (key, id) => (
                    ...
                    <li key={key} id={id} class="clearfix no-bullets">
                      ...
                      <div class="message other-message float-right">
                        {String(arr2[k][key][arr[i].name]).includes("data:image") ? <img class="img-link"
                         src={arr2[k][key][arr[i].name]} alt="image" />
                          :
                          (arr2[k][key].isImage === true ? <div><Linkify>{arr2[k][key][arr[i].name]}</Linkify>
                            <div><img class="img-link" src={arr2[k][key][arr[i].name]} alt="image" />
                            </div></div>
                    ...
                    Object.keys(arr2[k][key])[0] === arr[i].name ?
                      (<li key={key} id={id} class="clearfix">
                        ...
                        <div class="message my-message">
                          {String(arr2[k][key][Object.keys(arr2[k][key])[0]]).includes("data:image") 
                          ? <img class="img-link" src={arr2[k][key][Object.keys(arr2[k][key])[0]]} alt="image" />
                            : (arr2[k][key].isImage === true ? <div>
                              <Linkify>{arr2[k][key][Object.keys(arr2[k][key])[0]]}</Linkify>
                              <div><img class="img-link" 
                              src={arr2[k][key][Object.keys(arr2[k][key])[0]]} alt="image" />
                              </div></div>
                      ...
                      (<li key={key} id={id} class="clearfix">
                        ...
                        <div class="message other-message float-right">
                          {String(arr2[k][key][arr[i].name]).includes("data:image") ? <img class="img-link"
                           src={arr2[k][key][arr[i].name]} alt="image" />
                            :(arr2[k][key].isImage === true ? <div>
                              <Linkify>{arr2[k][key][arr[i].name]}</Linkify>
                              <div><img class="img-link" src={arr2[k][key][arr[i].name]} alt="image" />
                              </div></div>
                              : <div>{arr2[k][key][arr[i].name]}</div>)}
                        </div>
          ...
          chatControl = (<div class="chat-message clearfix">
            <textarea onChange={(e) => this.onChangeMessage(e)} name="messageSend" id="messageSend" 
